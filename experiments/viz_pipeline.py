...
def storyboard(NAME, outdir):
    return _storyboard(NAME, outdir)


def _storyboard(NAME, outdir):
        # ---------------- metadata + input
    ...
    path = os.path.join(outdir, f'viz_{NAME}.png')
...
if __name__ == '__main__':
    import glob
    outdir = os.path.join(OUT, 'viz')
    os.makedirs(outdir, exist_ok=True)
    if len(sys.argv) > 1 and sys.argv[1] != '--all':
        storyboard(sys.argv[1], outdir)
    else:
        metas = sorted(glob.glob(os.path.join(CH1, '*.png.txt')))
        metas = [m for m in metas
                 if os.path.exists(m[:-8] + '-mask.png')][::4][:12]
        for meta in metas:
            name = os.path.basename(meta)[:-8]
            try:
                storyboard(name, outdir)
